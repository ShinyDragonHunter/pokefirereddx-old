...
static bool8 Task_ProcessSoundCheckMenuInput(u8 taskId) // sub_080E8688
{
    ...
    else if (JOY_REPEAT(DPAD_LEFT) != TEST_MUS)
    {
        if (gTasks[taskId].tWindowSelected)
        {
        ...
        else
        {
            if (gTasks[taskId].tBgmIndex > 0)
                ...
                gTasks[taskId].tBgmIndex = (MUS_RG_TEACHY_TV_MENU - (MUS_LITTLEROOT_TEST - 1));
        }
        return TRUE;
    }
    else if (JOY_REPEAT(DPAD_RIGHT) != TEST_MUS)
    {
        if (gTasks[taskId].tWindowSelected)
        {
        ...
        {
            if (gTasks[taskId].tBgmIndex < (MUS_RG_TEACHY_TV_MENU - (MUS_LITTLEROOT_TEST - 1)))
                gTasks[taskId].tBgmIndex++;
            else
                gTasks[taskId].tBgmIndex = 0;
        }
...
static void Task_DrawSubmenu(u8 taskId) // sub_080E89EC
{
    ...
    case 2:
        ...
        gTasks[taskId].func = (gTasks[taskId].tWhichSubmenu) ? Task_DrawDriverTestMenu : Task_DrawPanTestMenu;
        break;
    }
}
...
static void Task_DrawDriverTestMenu(u8 taskId) // sub_080E8AA0
{
    ...
    InitMenuInUpperLeftCorner(WIN_INFO, 2, 0, 0, 16, 9, 0, 0);
...
static void Task_ProcessDriverTestInput(u8 taskId) // sub_080E8D68
{
    ...
    if (JOY_NEW(A_BUTTON))
    {
        ...
        remaining = sSoundTestParams[CRY_TEST_VOICE];
        sMPlayInfo_Cry = SetPokemonCryTone(sDriverTest_Reverse ? &gCryTable2[remaining] : &gCryTable[remaining]);
    }
    ...
    for (i = digits - 1; i >= 0; i--)
    {
        ...
        if (d || someVar2 || !i)
        {
            if (negative && !someVar2)
                str[digits - i] = CHAR_HYPHEN;
            str[digits - i - 1] = CHAR_0 + d;
            someVar2 = TRUE;
        }
        n %= powersOfTen[i];
    }
...
static void Task_ProcessPanTestInput(u8 taskId) // sub_080E9284
{
    ...
    else if (JOY_REPEAT(DPAD_LEFT))
    {
        ...
        if (sSoundTestParams[CRY_TEST_VOICE] < MUS_DUMMY)
            sSoundTestParams[CRY_TEST_VOICE] = 270;
        PrintPanTestMenuText();
    }
}

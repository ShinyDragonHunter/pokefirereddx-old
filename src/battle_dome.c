...
static void CreateDomeOpponentMon(u8 monPartyId, u16 tournamentTrainerId, u8 tournamentMonId, u32 otId)
{
    ...
    CreateMonWithEVSpreadNatureOTID(&gEnemyParty[monPartyId],
                                         ...
                                         fixedIv,
                                         gFacilityTrainerMons[DOME_MONS[tournamentTrainerId][tournamentMonId]].evSpread, otId, 0);
...
static int GetTypeEffectivenessPoints(int move, int targetSpecies, int arg2)
{
    ...
    if (!move || move == 0xFFFF || !gBattleMoves[move].power)
        return 0;
...
static u8 Task_GetInfoCardInput(u8 taskId)
{
    ...
    if (gTasks[taskId].data[3] == INFOCARD_TRAINER)
    {
        ...
        if (JOY_NEW(DPAD_UP) && !sInfoCard->pos)
        {
            if (position)
                position--;
            else
        ...
        else if (JOY_NEW(DPAD_DOWN) && !sInfoCard->pos)
        {
        ...
        else if (JOY_NEW(DPAD_LEFT) && sInfoCard->pos)
        {
            sInfoCard->pos--;
    ...
    else // gTasks[taskId].data[3] == INFOCARD_MATCH
    {
        ...
        if (input == INFOCARD_INPUT_AB)
        {
            if (!sInfoCard->pos) // On left trainer info card
...
static void DisplayTrainerInfoOnCard(u8 flags, u8 trainerTourneyId)
{
    ...
    if (trainerId == TRAINER_PLAYER)
        sInfoCard->spriteIds[arrId] = CreateTrainerPicSprite(PlayerGenderToFrontTrainerPicId(gSaveBlock2Ptr->playerGender, gSaveBlock2Ptr->playerOutfit), TRUE, x + 48, y + 64, palSlot + 12, TAG_NONE);
...
static void DisplayMatchInfoOnCard(u8 flags, u8 matchNo)
{
    ...
    if (trainerIds[0] == TRAINER_PLAYER)
        sInfoCard->spriteIds[arrId] = CreateTrainerPicSprite(PlayerGenderToFrontTrainerPicId(gSaveBlock2Ptr->playerGender, gSaveBlock2Ptr->playerOutfit), TRUE, x + 48, y + 88, palSlot + 12, TAG_NONE);
    ...
    if (trainerIds[1] == TRAINER_PLAYER)
        sInfoCard->spriteIds[1 + arrId] = CreateTrainerPicSprite(PlayerGenderToFrontTrainerPicId(gSaveBlock2Ptr->playerGender, gSaveBlock2Ptr->playerOutfit), TRUE, x + 192, y + 88, palSlot + 13, TAG_NONE);
...
static void InitRandomTourneyTreeResults(void)
{
    int i, j, k, monLevel;
    int species[FRONTIER_PARTY_SIZE];
    int monTypesBits, trainerId, monId, zero1, zero2;
    u8 lvlMode;
    u16 *statSums;
    int *statValues;
    u8 ivs = 0;
...
static void DecideRoundWinners(u8 roundId)
{
    int i, moveSlot, monId1, monId2, tournamentId1, tournamentId2, species;

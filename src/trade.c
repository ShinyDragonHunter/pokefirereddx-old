...
static bool8 BufferTradeParties(void)
{
    ...
    case 12:
        ...
        break;
    case 13:
        memcpy(gBlockSendBuffer, gSaveBlock1Ptr->mail, PARTY_SIZE * sizeof(struct MailStruct) + 4);
        sTradeMenuData->bufferPartyState++;
        break;
    case 15:
        if (id == 0)
    ...
    case 16:
        if (_GetBlockReceivedStatus() == 3)
        {
            memcpy(gTradeMail, gBlockRecvBuffer[id ^ 1], PARTY_SIZE * sizeof(struct MailStruct));
            TradeResetReceivedFlags();
            sTradeMenuData->bufferPartyState++;
        }
        break;
    case 17:
        memcpy(gBlockSendBuffer, gSaveBlock1Ptr->giftRibbons, sizeof(sTradeMenuData->giftRibbons));
        sTradeMenuData->bufferPartyState++;
        break;
    case 19:
        if (id == 0)
    ...
    case 20:
        if (_GetBlockReceivedStatus() == 3)
        {
            memcpy(sTradeMenuData->giftRibbons, gBlockRecvBuffer[id ^ 1], sizeof(sTradeMenuData->giftRibbons));
            TradeResetReceivedFlags();
            sTradeMenuData->bufferPartyState++;
        }
        break;
...
static bool8 AnimateTradeSequenceCable(void)
{
    ...
    case TS_STATE_TRY_EVOLUTION: // Only if in-game trade, link trades use CB2_TryLinkTradeEvolution
        ...
        if (evoTarget)
        {
            TradeEvolutionScene(&gPlayerParty[gSelectedTradeMonPositions[TRADE_PLAYER]], evoTarget, sTradeData->monSpriteIds[TRADE_PARTNER], gSelectedTradeMonPositions[TRADE_PLAYER], formTarget);
        }
        sTradeData->state++;
        break;

...
void BedroomPC(void)
{
    gPlayerPCItemPageInfo.notInRoom = FALSE;
...
void PlayerPC(void)
{
    gPlayerPCItemPageInfo.notInRoom = TRUE;
    sTopMenuOptionOrder = sPlayerPC_OptionOrder;
    sTopMenuNumOptions = NUM_PLAYER_PC_OPTIONS;
    DisplayItemMessageOnField(CreateTask(TaskDummy, 0), gText_WhatWouldYouLike, InitPlayerPCMenu);
}
...
static void PlayerPC_TurnOff(u8 taskId)
{
    if (gPlayerPCItemPageInfo.notInRoom)
...
static void Mailbox_DrawMailboxMenu(u8 taskId)
{
    ...
    windowId = MailboxMenu_AddWindow(MAILBOXWIN_TITLE);
...
static void ItemStorage_DrawSwapArrow(u8 y, u8 b, u8 speed)
{
    ...
    if (b == 0xFF)
        ...
        AddTextPrinterParameterized4(windowId, 2, 0, y, 0, 0, sSwapArrowTextColors, speed, gText_SelectorArrow2);
}
...
static void ItemStorage_CreateListMenu(u8 taskId)
{
    ...
    AddTextPrinterParameterized(sItemStorageMenu->windowIds[ITEMPC_WIN_TITLE], 2, text, x, 1, 0, NULL);

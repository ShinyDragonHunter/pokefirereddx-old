...
static const u8 *const sPassAreaDescriptions[CURSOR_AREA_COUNT + 1] =
{
    ...
    [CURSOR_AREA_COUNT]          = gText_ExpandedPlaceholder_Empty,
};
...
static u8 MapNumToFrontierFacilityId(u16 mapNum) // id + 1, zero means not a frontier map number
{
    ...
    // In Battle Dome
    else if (mapNum == MAP_NUM(BATTLE_FRONTIER_BATTLE_DOME_LOBBY) && mapNum <= MAP_NUM(BATTLE_FRONTIER_BATTLE_DOME_BATTLE_ROOM))
        return FRONTIER_FACILITY_DOME + 1;

    // In Battle Palace
    else if (mapNum >= MAP_NUM(BATTLE_FRONTIER_BATTLE_PALACE_LOBBY) && mapNum <= MAP_NUM(BATTLE_FRONTIER_BATTLE_PALACE_BATTLE_ROOM))
        return FRONTIER_FACILITY_PALACE + 1;

    // In Battle Arena
    else if (mapNum >= MAP_NUM(BATTLE_FRONTIER_BATTLE_ARENA_LOBBY) && mapNum <= MAP_NUM(BATTLE_FRONTIER_BATTLE_ARENA_BATTLE_ROOM))
        return FRONTIER_FACILITY_ARENA + 1;

    // In Battle Factory
    else if (mapNum >= MAP_NUM(BATTLE_FRONTIER_BATTLE_FACTORY_LOBBY) && mapNum <= MAP_NUM(BATTLE_FRONTIER_BATTLE_FACTORY_BATTLE_ROOM))
        return FRONTIER_FACILITY_FACTORY + 1;

    // In Battle Pyramid
    else if (mapNum >= MAP_NUM(BATTLE_FRONTIER_BATTLE_PIKE_LOBBY) && mapNum <= MAP_NUM(BATTLE_FRONTIER_BATTLE_PIKE_ROOM_WILD_MONS))
        return FRONTIER_FACILITY_PIKE + 1;

    else if (mapNum >= MAP_NUM(BATTLE_FRONTIER_BATTLE_PYRAMID_LOBBY) && mapNum <= MAP_NUM(BATTLE_FRONTIER_BATTLE_PYRAMID_TOP))

...
static void CB2_InitPartyMenu(void)
{
    while (TRUE)
    {
        if (MenuHelpers_CallLinkSomething() || ShowPartyMenu() || MenuHelpers_LinkSomething())
            break;
    }
}
...
static bool8 DisplayPartyPokemonDataForMoveTutorOrEvolutionItem(u8 slot)
{
    ...
    else
    {
        ...
        case 2: // Evolution stone
            if (!GetMonData(currentPokemon, MON_DATA_IS_EGG) && GetEvolutionTargetSpecies(currentPokemon, EVO_MODE_ITEM_CHECK, item, &targetForm) != SPECIES_NONE)
                return FALSE;
...
{
    u8 confirmWindowId, cancelWindowId, offset, mainOffset;
    ...
    if (gPartyMenu.menuType != PARTY_MENU_TYPE_MULTI_SHOWCASE)
    {
        if (chooseHalf)
        {
            ...
            mainOffset = GetStringCenterAlignXOffset(4, gMenuText_Confirm, 48);
            AddTextPrinterParameterized4(confirmWindowId, 4, mainOffset, 1, 0, 0, sFontColorTable[0], -1, gMenuText_Confirm);
        ...
        mainOffset = GetStringCenterAlignXOffset(4, gText_Cancel, 48);
        AddTextPrinterParameterized3(cancelWindowId, 4, mainOffset + offset, 1, sFontColorTable[0], -1, gText_Cancel);
...
{
    AddTextPrinterParameterized3(windowId, 4, align[0], align[1], sFontColorTable[color], 0, str);
}
...
static void DisplayPartyPokemonGender(u8 gender, u16 species, u8 *nickname, struct PartyMenuBox *menuBox)
{
    ...
    if (species == SPECIES_NONE)
        return;
    if ((species == SPECIES_NIDORAN_M || species == SPECIES_NIDORAN_F) && !StringCompare(nickname, gSpeciesNames[species]))
        return;
    switch (gender)
    {
    case MON_MALE:
...
static u8 DisplaySelectionWindow(u8 windowType)
{
    ...
    u8 cursorDimension, fontAttribute;
    u32 i;
...
static void Task_SlideSelectedSlotsOnscreen(u8 taskId)
{
    ...
    if (!tSlot1SlideDir && !tSlot2SlideDir)
    {
        PutWindowTilemap(sPartyMenuBoxes[gPartyMenu.slotId].windowId);
        PutWindowTilemap(sPartyMenuBoxes[gPartyMenu.slotId2].windowId);
        ScheduleBgCopyTilemapToVram(0);
...
static void CursorCb_FieldMove(u8 taskId)
{
    ...
    else
    {
        ...
        else if (sFieldMoveCursorCallbacks[fieldMove].fieldMoveFunc())
        {
            switch (fieldMove)
            {
...
static bool8 SetUpFieldMove_Waterfall(void)
{
    ...
    if (MetatileBehavior_IsWaterfall(MapGridGetMetatileAttributeAt(x, y, METATILE_ATTRIBUTE_BEHAVIOR)) && IsPlayerSurfingNorth())
    {
        gFieldCallback2 = FieldCallback_PrepareFadeInFromMenu;
...
static void CreatePartyMonIconSprite(struct Pokemon *mon, struct PartyMenuBox *menuBox, u32 slot)
{
    ...
    CreatePartyMonIconSpriteParameterized(species2, GetMonData(mon, MON_DATA_PERSONALITY), menuBox, 1, GetMonData(mon, MON_DATA_FORM));
    UpdatePartyMonHPBar(menuBox->monSpriteId, mon);
}

static void CreatePartyMonIconSpriteParameterized(u16 species, u32 pid, struct PartyMenuBox *menuBox, u8 priority, u8 form)
{
    if (species)
    {
        menuBox->monSpriteId = CreateMonIcon(species, SpriteCB_MonIcon, menuBox->spriteCoords[0], menuBox->spriteCoords[1], 4, pid, form);
...
static void ShowMoveSelectWindow(u8 slot)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        AddTextPrinterParameterized(windowId, fontId, gMoveNames[move], 8, (i * 16) + 2, TEXT_SPEED_FF, NULL);
        if (move != MOVE_NONE)
            moveCount++;
    }
...
static void PartyMenuTryEvolution(u8 taskId)
{
    u8 targetForm;
    struct Pokemon *mon = &gPlayerParty[gPartyMenu.slotId];
    u16 targetSpecies = GetEvolutionTargetSpecies(mon, EVO_MODE_NORMAL, ITEM_NONE, &targetForm);

    if (targetSpecies)
    {
        FreePartyPointers();
        gCB2_AfterEvolution = gPartyMenu.exitCallback;
        BeginEvolutionScene(mon, targetSpecies, TRUE, gPartyMenu.slotId, targetForm);
        DestroyTask(taskId);
    }
    else
        gTasks[taskId].func = Task_ClosePartyMenuAfterText;

...
extern const struct SpritePalette gMonPaletteTable[]; // GF made a mistake and did not extern it as const.
...
{
    s32 spriteId = CreateMonPicSprite(species, 0, 0x8000, 1, x, y, 0, gMonPaletteTable[species].tag, 0);
...
u8 CreateMonSprite_FieldMove(u16 species, u32 otId, u32 personality, s16 x, s16 y, u8 subpriority)
{
    ...
    u16 spriteId = CreateMonPicSprite(species, otId, personality, 1, x, y, 0, spritePalette->tag, 0);
...
u8 FldEff_UseSurf(void)
{
    ...
    Overworld_ChangeMusicTo(gSurfMusicTable[gMapsecToRegion[gMapHeader.regionMapSectionId]]);
    return FALSE;
}

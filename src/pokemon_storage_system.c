...
static struct Sprite *CreateMonIconSprite(u16, u32, s16, s16, u8, u8, u8);
...
static void Task_ReshowPokeStorage(u8 taskId)
{
    ...
    case 1:
        ...
        {
            if (sWhichToReshow == SCREEN_CHANGE_ITEM_FROM_BAG - 1 && gSpecialVar_ItemId)
            {
...
static void Task_OnSelectedMon(u8 taskId)
{
    ...
    case 0:
        if (!IsDisplayMosaicActive())
        {
            ...
            else if (IsMovingItem() || sStorage->displayMonItemId)
...
static void CreateMovingMonIcon(void)
{
    ...
    sStorage->movingMonSprite = CreateMonIconSprite(species, personality, 0, 0, priority, 7, form);
    ...
    for (i = 0; i < IN_BOX_ROWS; i++)
    {
        for (j = 0; j < IN_BOX_COLUMNS; j++)
        {
            ...
            if (species)
            {
                ...
                sStorage->boxMonsSprites[count] = CreateMonIconSprite(species, personality, 8 * (3 * j) + 100, 8 * (3 * i) + 44, 2, 19 - j, form);
            }
            else
            {
...
static void CreateBoxMonIconAtPos(u8 boxPosition)
{
    ...
    if (species)
    {
        ...
        sStorage->boxMonsSprites[boxPosition] = CreateMonIconSprite(species, personality, x, y, 2, 19 - (boxPosition % IN_BOX_ROWS), form);
        if (sStorage->boxOption == OPTION_MOVE_ITEMS)
            sStorage->boxMonsSprites[boxPosition]->oam.objMode = ST_OAM_OBJ_BLEND;
    }
}
...
{
    if (sStorage->iconScrollDistance)
    ...
    case 2:
        ...
        if (!sStorage->iconScrollNumIncoming)
        {
            sStorage->iconScrollDistance++;
            return FALSE;
        }
...
static void CreatePartyMonsSprites(bool8 visible)
{
    ...
    sStorage->partySprites[0] = CreateMonIconSprite(species, personality, 104, 64, 1, 12, form);
    count = 1;
    for (i = 1; i < PARTY_SIZE; i++)
    {
    ...
    if (sStorage->boxOption == OPTION_MOVE_ITEMS)
    {
        for (i = 0; i < PARTY_SIZE; i++)
        {
            if (sStorage->partySprites[i] && !GetMonData(&gPlayerParty[i], MON_DATA_HELD_ITEM))
...
{
    if (sStorage->partySprites[partyId])
    {
...
static void SetReleaseMon(u8 mode, u8 position)
{
    ...
    if (*sStorage->releaseMonSpritePtr)
    {
...
static void LoadWallpaperGfx(u8 boxId, s8 direction)
{
    ...
    if (sStorage->wallpaperLoadDir)
    {
        sStorage->wallpaperOffset = (!sStorage->wallpaperOffset);
...
static bool8 UpdateCursorPos(void)
{
    ...
    else if (--sStorage->cursorMoveSteps)
    {
...
static void InitCursorMove(void)
{
    ...
    if (sStorage->cursorVerticalWrap || sStorage->cursorHorizontalWrap)
...
static bool8 IsRemovingLastPartyMon(void)
{
    if (sCursorArea == CURSOR_AREA_IN_PARTY && !sIsMonBeingMoved && !CountPartyAliveNonEggMonsExcept(sCursorPosition))
        return TRUE;
    else
        return FALSE;
}
...
static bool8 CanShiftMon(void)
{
    if (sIsMonBeingMoved)
    {
        if (sCursorArea == CURSOR_AREA_IN_PARTY && !CountPartyAliveNonEggMonsExcept(sCursorPosition))
        {
            if (sStorage->displayMonIsEgg || !GetMonData(&sStorage->movingMon, MON_DATA_HP))
                return FALSE;
        }
        return TRUE;
    }
...
static void SetDisplayMonData(void *pokemon, u8 mode)
{
    ...
    else if (mode == MODE_BOX)
    {
        ...
        if (sStorage->displayMonSpecies)
        {
            ...
            sStorage->displayMonShiny = IsShinyOtIdPersonality(otId, sStorage->displayMonPersonality);
        }
    }
    else
...
static u8 InBoxInput_Normal(void)
{
    ...
    do
    {
        ...
        {
            retVal = INPUT_MOVE_CURSOR;
            if ((sCursorPosition + 1) % IN_BOX_COLUMNS)
            {
                cursorPosition++;
            }
            else
...
static u8 InBoxInput_SelectingMultiple(void)
{
    ...
    else
    {
        ...
        {
            sIsMonBeingMoved = (sStorage->displayMonSpecies);
...
static u8 HandleInput_InParty(void)
{
    ...
    do
    {
        ...
        else if (JOY_REPEAT(DPAD_LEFT) && sCursorPosition)
        {
            retVal = INPUT_MOVE_CURSOR;
...
static bool8 UpdateItemInfoWindowSlideIn(void)
{
    ...
    return (sStorage->itemInfoWindowOffset);
}

...
static void Task_Hof_DisplayMon(u8 taskId)
{
    ...
    spriteId = CreatePicSprite2(currMon->species, currMon->tid, currMon->personality, 1, startX, startY, currMonId, TAG_NONE, 0);
...
static void Task_Hof_DisplayPlayer(u8 taskId)
{
    ...
    ShowBg(1);
    ShowBg(3);
    gTasks[taskId].tPlayerSpriteID = CreateTrainerPicSprite(PlayerGenderToFrontTrainerPicId(gSaveBlock2Ptr->playerGender, gSaveBlock2Ptr->playerOutfit), 1, 120, 72, 6, TAG_NONE);
...
static void Task_HofPC_DrawSpritesPrintText(u8 taskId)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++, currMon++)
    {
        if (currMon->species != 0)
        {
            ...
            if (currMon->species == SPECIES_EGG)
                posY += 10;
            spriteId = CreateMonPicSprite(currMon->species, currMon->tid, currMon->personality, 1, posX, posY, i, TAG_NONE, 0);
...
static void HallOfFame_PrintMonInfo(struct HallofFameMon* currMon)
{
    ...
    {
        width = GetStringRightAlignXOffset(2, text, 0x80);
        AddTextPrinterParameterized3(0, 2, width, 1, sMonInfoTextColors, -1, text);
...
static void HallOfFame_PrintPlayerInfo(void)
{
    ...
    width = GetStringRightAlignXOffset(2, gSaveBlock2Ptr->playerName, 0x70);
    ...
    text[5] = EOS;
    width = GetStringRightAlignXOffset(2, text, 0x70);

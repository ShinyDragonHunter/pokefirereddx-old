...
static u8 GetLastAlphabetColumn(u8 row)
{
    ...
    case 1:
        return NUM_ALPHABET_COLUMNS - 2; // At 6 letters, only the 2nd row (index 1) has less than the max columns
                                         // The 3rd and 4th row have 7 letters, the 1st row has 6 letters and 'Other
    default:
...
static bool8 RunEasyChatFunction(void)
{
    switch (sScreenControl->currentFuncId)
    {
        ...
        case ECFUNC_NONE:
        case ECFUNC_EXIT:
        case ECFUNC_QUIZ_QUESTION:      // The 4 quiz functions
        case ECFUNC_QUIZ_ANSWER:        // 'finish' automatically
        case ECFUNC_SET_QUIZ_QUESTION: // because they switch to a
        case ECFUNC_SET_QUIZ_ANSWER:   // callback in sQuizLadyEasyChatScreens 
        default:
            return FALSE;
    }
}
...
static void TryAddInterviewObjectEvents(void)
{
    ...
    spriteId = AddPseudoObjectEvent(
        gSaveBlock2Ptr->playerGender ? OBJ_EVENT_GFX_LEAF_NORMAL : OBJ_EVENT_GFX_RED_NORMAL,
        SpriteCallbackDummy,
        52,
        40,
        0);

...
static void UpdateMapSecInfoWindow(struct Pokenav5Struct_2 *state)
{
    ...
    case MAPSECTYPE_CITY_CANFLY:
        ...
        AddTextPrinterParameterized(state->infoWindowId, 5, regionMap->mapSecName, 0, 1, TEXT_SPEED_FF, NULL);
...
static void PrintLandmarkNames(struct Pokenav5Struct_2 *state, int mapSecId, int pos)
{
    ...
    while (1)
    {
        ...
        AddTextPrinterParameterized(state->infoWindowId, 5, gStringVar1, 0, i * 16 + 17, TEXT_SPEED_FF, NULL);
        i++;
    }
}

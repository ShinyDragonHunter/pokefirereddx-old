...
static void (*const sRecordedPlayerBufferCommands[CONTROLLER_CMDS_COUNT])(void) =
{
    ...
    [CONTROLLER_DEBUGMENU]                = RecordedPlayerEndExecution,
    ...
    case REQUEST_ALL_BATTLE:
         ...
         && battleMon.species == SPECIES_DEOXYS)
        {
            if ((gLinkPlayers[gRecordedBattleMultiplayerId].version & 0xFF) < VERSION_EMERALD)
                ...
                battleMon.form = (gLinkPlayers[gRecordedBattleMultiplayerId].versionModifier == MODIFIER_CRYSTALDUST) ? SPEED : ATTACK;
    ...
    case REQUEST_SPECIES_BATTLE:
         ...
         && data16 == SPECIES_DEOXYS)
        {
            if ((gLinkPlayers[gRecordedBattleMultiplayerId].version & 0xFF) < VERSION_EMERALD)
                ...
                form = (gLinkPlayers[gRecordedBattleMultiplayerId].versionModifier == MODIFIER_CRYSTALDUST) ? SPEED : ATTACK;
            else if ((gLinkPlayers[gRecordedBattleMultiplayerId].version & 0xFF) == VERSION_LEAF_GREEN)
                form = DEFENSE;
        }
        dst[0] = data16;
...
static void RecordedPlayerHandleDrawTrainerPic(void)
{
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_INGAME_PARTNER)
    {
        trainerPicId = gPlayerFrontPics[0][trainerPicId];

...
static void HandleInputChooseMove(void)
{
    ...
    else if (JOY_NEW(SELECT_BUTTON))
    {
        if (gNumberOfMovesToChoose > 1 && !(gBattleTypeFlags & BATTLE_TYPE_LINK))
        {
            ...
            if (gMoveSelectionCursor[gActiveBattler])
...
{
    if (JOY_NEW(DPAD_UP) && gMultiUsePlayerCursor)
    {
        PlaySE(SE_SELECT);
...
static void MoveSelectionDisplayPpNumber(void)
{
    ...
    if (gBattleBufferA[gActiveBattler][2]) // check if we didn't want to display pp number
        return;
...
static u32 CopyPlayerMonData(u8 monId, u8 *dst)
{
    ...
    case REQUEST_ALL_BATTLE:
        battleMon.species = GetFormSpecies(GetMonData(&gPlayerParty[monId], MON_DATA_SPECIES),
                                        GetMonData(&gPlayerParty[monId], MON_DATA_FORM));
    ...
    case REQUEST_SPECIES_BATTLE:
        data16 = GetFormSpecies(GetMonData(&gPlayerParty[monId], MON_DATA_SPECIES),
                                GetMonData(&gPlayerParty[monId], MON_DATA_FORM));
        dst[0] = data16;
        dst[1] = data16 >> 8;
        size = 2;
